...
// AntigravityControlCenter.qml — Antigravity usage grouped by day, week, month.
// Separate from TokenControlCenter: no cost, no cache write, own monitor type.
...
Item {
  ...
  property AntigravityUsage monitor: null
  ...
  readonly property var sourceRows: root.monitor === null ? [] : root.monitor.monthSources.map(function (s) {
    return {
      name: s.name,
      tokens: root.monitor.compact(s.tokens),
      sub: root.monitor.compact(s.messages) + " msgs"
    };
  })

  function splitDetails(split: var, messages: double): var {
    return [
  ...
  readonly property var rows: root.monitor === null ? [] : [
    {
      ...
      details: root.splitDetails(root.monitor.todaySplit, root.monitor.todayMessages)
    },
    {
      label: "This week",
      tokens: root.monitor.compact(root.monitor.weekTokens),
      details: root.splitDetails(root.monitor.weekSplit, root.monitor.weekMessages)
    },
    {
      label: "This month",
      tokens: root.monitor.compact(root.monitor.monthTokens),
      details: root.splitDetails(root.monitor.monthSplit, root.monitor.monthMessages)
    }
  ]
  ...
  Rectangle {
    ...
    ColumnLayout {
      ...
      ColumnLayout {
        Layout.fillWidth: true
        spacing: 0
        visible: root.sourceRows.length > 0
          ...
          delegate: ColumnLayout {
            ...
            RowLayout {
              ...
              ColumnLayout {
                ...
                Text {
                  text: sourceDelegate.modelData.tokens
                  color: "#ffffff"
                  font.pixelSize: 13
                  font.family: "JetBrainsMono Nerd Font Mono"
                  horizontalAlignment: Text.AlignRight
                ...
                Text {
                  text: sourceDelegate.modelData.sub
                  color: "#6F6F84"
                  font.pixelSize: 11
                  font.family: "JetBrainsMono Nerd Font Mono"
                  horizontalAlignment: Text.AlignRight
