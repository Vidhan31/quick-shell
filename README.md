...
- `services/`. Singletons and long-running state managers, such as the desktop notification service (Quickshell 0.3.1 `NotificationServer` + QML history/toast models) and the media service (official `Quickshell.Services.Mpris` + QML active-player policy).
